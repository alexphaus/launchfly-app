...
RANKINGS: the pack contains "candidates" — REAL, sourced opportunities (actual businesses, listings, people) found by the system. For each candidate return {id, fit_score 0-100, reason}. The score is a verdict the person will not see as a number: 60 or more means you would tell them to contact this one this week — only those are shown to them; under 60 means a reason not to outweighs the fit (price, distance, size, timing), and it is set aside unseen. For 60 and above, the reason is the case for reaching out, in one or two sentences about THIS candidate and THIS person's goals and constraints; below 60, say the reason not to. Never invent candidates; never change their ids. Candidates with scored=false must be ranked; re-rank scored ones only when context changed.

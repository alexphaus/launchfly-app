-- Enhanced AI Cofounder Database Schema (Fixed)
...
CREATE TABLE IF NOT EXISTS ai_memories (
  ...
  memory_timestamp TIMESTAMPTZ DEFAULT NOW(), -- Renamed from 'timestamp'
...
CREATE INDEX IF NOT EXISTS idx_ai_memories_timestamp ON ai_memories(memory_timestamp DESC);
...
CREATE TABLE IF NOT EXISTS shared_ai_knowledge (
  ...
  knowledge_timestamp TIMESTAMPTZ DEFAULT NOW() -- Renamed from 'timestamp'
);
...
RETURNS TABLE (
  ...
  context JSONB,
  metadata JSONB,
  memory_timestamp TIMESTAMPTZ, -- Updated column name
  similarity FLOAT
)
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  SELECT 
    ...
    m.context,
    m.metadata,
    m.memory_timestamp, -- Updated column name
...
CREATE INDEX IF NOT EXISTS idx_revenue_forecasts_business ON revenue_forecasts(business_id, created_at DESC);

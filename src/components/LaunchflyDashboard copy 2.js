...
import { ChevronRight, Globe, DollarSign, Users, Rocket, Check, TrendingUp, Sparkles, Clock, X, AlertCircle, MessageSquare, LayoutGrid, List, Settings, Bot, Inbox } from 'lucide-react';
    ...
    return (
        <div className="bg-white rounded-2xl p-6 md:p-8" style={{ boxShadow: theme.shadows.lg }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {steps.map((step) => (
                    <div key={step.id} className="p-6 rounded-xl border border-gray-200 flex flex-col">
                        <div className="text-4xl mb-4">{step.icon}</div>
                        <h4 className="text-lg font-bold mb-2" style={{ color: theme.colors.textDark }}>{step.title}</h4>
                        <p className="text-sm mb-5 min-h-[40px] flex-grow" style={{ color: theme.colors.textGray }}>{step.description}</p>
                        <button
...
const MoneyBar = ({ totalRevenue, payoutBalance }) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <button 
            onClick={() => { if(window.confirm('Are you sure you want to cash out?')) alert('Payout initiated!'); }}
            className="bg-green-500 rounded-2xl p-6 flex items-center justify-center text-white font-bold text-xl cursor-pointer hover:bg-green-600 transition-all" style={{ boxShadow: theme.shadows.lg }}>
...
const TabbedCommandCenter = ({ activeTab, setActiveTab, businessData }) => {
    ...
    return (
        <div className="bg-white rounded-2xl" style={{ boxShadow: theme.shadows.lg }}>
            <div className="border-b overflow-x-auto" style={{ borderColor: theme.colors.borderLight }}>
                <nav className="flex gap-2 p-2">
                    {tabs.map(tab => (
                        <button
                            key={tab.name}
                            onClick={() => setActiveTab(tab.name.toLowerCase())}
                            className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all ${activeTab === tab.name.toLowerCase() ? 'text-white' : ''}`}
                            style={{
    ...
    return (
        <div className="text-center py-12">
            <div className="mx-auto w-16 h-16 flex items-center justify-center rounded-full bg-gray-100 mb-4">
                <Icon className="w-8 h-8" style={{color: theme.colors.textGray}} />
            </div>
            <h3 className="text-lg font-bold" style={{color: theme.colors.textDark}}>{title}</h3>
            <p className="text-sm" style={{color: theme.colors.textGray}}>{message}</p>
        </div>
    );
};
...
const SalesView = ({ sales }) => {
    if (sales.length === 0) {
        return <EmptyState icon={Inbox} title="No Sales Yet" message="Your first sale will appear here as soon as it happens." />;
    }
    return (
        <div>
...
const ProductsView = ({ products }) => {
    if (products.length === 0) {
        return <EmptyState icon={List} title="No Products Found" message="Your AI is creating your products. They will appear here shortly." />;
    }
    return (
        <div>
            ...
            <div className="space-y-4">
                {products.map(product => (
                    <div key={product.id} className="flex flex-col sm:flex-row justify-between sm:items-center p-4 rounded-lg border" style={{borderColor: theme.colors.borderLight}}>
                        ...
                        <div className="flex items-center gap-4 flex-shrink-0">
                            ...
                            <button className="px-3 py-1 text-sm font-semibold rounded-md border hover:bg-gray-50 transition-colors" style={{borderColor: theme.colors.borderLight}}>Edit</button>
                        </div>
                    </div>
                ))}
            </div>
...
const GrowthView = ({ growthData }) => (
    <div>
        ...
        <EmptyState icon={TrendingUp} title="Analyzing Growth" message="AI-powered growth analytics will appear here once you have more data." />
...
const SettingsView = () => (
    <div>
        ...
        <EmptyState icon={Settings} title="Coming Soon" message="Manage your business details and integrations here." />
    </div>
);
            ...
            {isOpen && (
                <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setIsOpen(false)}>
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 animate-slideUp" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-sm mb-4" style={{color: theme.colors.textGray}}>What would you like to change or know? Try one of these:</p>
                        <div className="flex flex-wrap gap-2 mb-4">
                            {["Change product price to $129", "Add a new product", "What were my sales yesterday?"].map(prompt => (
                                <button key={prompt} onClick={() => setMessage(prompt)} className="text-xs px-2 py-1 rounded-md bg-gray-100 hover:bg-gray-200">{prompt}</button>
                            ))}
                        </div>
                        <textarea
                            value={message}

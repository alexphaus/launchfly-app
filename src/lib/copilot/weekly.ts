...
export const WEEKLY_EYEBROW = "This week's openings";
...
export function composeWeekly(
  ...
  const body = `Across ${businesses} real ${businesses === 1 ? 'business' : 'businesses'} in your segments, what they have in common that nothing you send names:\n${lines}\n\nThese are conditions their listings show, not requests anyone made. Either name one in your openers, or stop matching the segments where it shows up. ${tail}`;
  const title = `${top.length} ${top.length === 1 ? 'thing' : 'things'} ${businesses} businesses in your segments have in common`;
...
export async function runWeeklySignals(profileId: string, opts: { now?: Date; force?: boolean } = {}): Promise<{ wrote: boolean; reason?: string }> {
  ...
  if (!composed) return { wrote: false, reason: 'no openings' };

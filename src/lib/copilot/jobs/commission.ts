...
export const commissionJob: Job = {
  ...
  async run(ctx: JobContext): Promise<MoveDraft[]> {
    ...
    // surfaces its ask in the same pass rather than tomorrow.
    ...
    for (const c of dueCommissions(all)) {
      ...
      try {
        ...
        await recordCommissionWork(ctx.profile.id, c, result);
        for (const e of result.events) byCommission.set(c.id, [...(byCommission.get(c.id) ?? []), { ...e, id: `pending:${c.id}:${e.summary.slice(0, 40)}`, commission_id: c.id, at: new Date().toISOString() }]);
      } catch (e) {

...
export async function GET(req: Request) {
   ...
   *
   * Deliberately counts finished work rather than taps: produced (jobs found
   * something) → called (the app picked one) → answered (you said what you did)
   * → landed (an outcome was recorded). Clicks would flatter it; these four
   * cannot. Every one is already logged, so this needed no tracking endpoint and
   * no client instrumentation.
   *
   * `nightly` is the number that explains the rest. With no cron there is no
   * overnight anything and produced/day is whatever the user generated by
   * opening the app.
   */
  const since = new Date(Date.now() - 30 * 86_400_000).toISOString();
  let loop: Record<string, number | null> | null = null;
  if (reachable) {
    const ev = (types: string[]) => db.from('copilot_events').select('id', { count: 'exact', head: true })
      .in('event_type', types).gte('created_at', since);
    const [produced, called, answered, landed, nightly] = await Promise.all([
      ev(['moves_written', 'moves_inbound']).then((r) => r.count ?? 0),
      ev(['decision_response']).then((r) => r.count ?? 0),
      ev(['move_answered', 'triage_answered']).then((r) => r.count ?? 0),
      db.from('copilot_outcomes').select('id', { count: 'exact', head: true }).gte('occurred_at', since).then((r) => r.count ?? 0),
      db.from('copilot_agent_runs').select('id', { count: 'exact', head: true })
        .eq('kind', 'daily_brief').filter('input_summary->>reason', 'eq', 'cron').gte('started_at', since)
        .then((r) => r.count ?? 0),
    ]).catch(() => [null, null, null, null, null] as Array<number | null>);
    loop = { produced, called, answered, landed, nightlyRuns: nightly };
  }

  return Response.json({
    ok: reachable && missing.length === 0 && schema.length === 0,
    // Zero nightly runs in thirty days is the single most useful line here: it
    // means nothing has happened while anyone was away, all month.
    loop,
